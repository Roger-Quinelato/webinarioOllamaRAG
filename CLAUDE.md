...
- **6.7** — ~~só `app.py` e `scripts/07_ollama.py` capturam `OllamaIndisponivel`; `scripts/03`–`06` e `opcional/*.py` não; a checagem `e6_ollama_desligado` só testa `rag.py`.~~ Resolvido: `scripts/03`–`07` e `opcional/*.py` já envolvem a execução em `with rag.cli_seguro():`, e `verificar.py e6_ollama_desligado_scripts` prova em runtime que o Ollama fora gera mensagem clara + `sys.exit(2)` sem traceback. A auditoria Gemini 3.1 Pro (AUD-002) achou a lacuna que sobrava: `scripts/01` e `02` ficavam fora do glob `0[3-7]` da checagem — o mesmo do ticket [#24](https://github.com/Roger-Quinelato/webinarioOllamaRAG/issues/24), já aberto (o AUD-002 duplicava o #24; a issue #55 criada pela auditoria foi fechada como duplicata do #24). Corrigida no [#24](https://github.com/Roger-Quinelato/webinarioOllamaRAG/issues/24): 01/02 agora envolvem `cli_seguro`, e a nova guarda estática `verificar.py e6_cli_seguro` varre `scripts/` + `opcional/` e falha (`sys.exit(1)`) se algum script que fala com o Ollama não envolver `cli_seguro`. Optou-se pela guarda estática em vez de estender `e6_ollama_desligado_scripts` a 01/02 (o que o #24 sugeria) porque rodar 02 com o Ollama fora zeraria o índice: `indexar()` recria a coleção antes de embutir. Evidência em `docs/evidencias/E6/aud002_cli_seguro.txt`.
...
- **5a.1, 7.2** — ~~números de evidência (ex.: tempo do SHAP) defasados em relação ao notebook versionado.~~ Resolvido no ticket [#8](https://github.com/Roger-Quinelato/webinarioOllamaRAG/issues/8) (T08): novo `verificar.py e7_saidas` lê `webinario_rag.ipynb` sem kernel e extrai por regex os tempos salvos (indexação, extração, resumo ao vivo, SHAP, resposta do bloco 6), para comparar contra o que os docs afirmam sem precisar reexecutar nada; usado para corrigir o tempo de SHAP em `docs/evidencias/E5/notebook_E5.txt` (34s → 48s, valor real da execução atual). **9.1** resolvido no ticket [#10](https://github.com/Roger-Quinelato/webinarioOllamaRAG/issues/10) (T10): as duas menções "na última execução do notebook" em `docs/medicoes.md` (bloco 4/SHAP, bloco 6/Ollama) reconciliadas com os valores reais (T09) e data; nova checagem `verificar.py e9_numeros` confere automaticamente todo número desse formato contra `E7/saidas_notebook.txt`; evidência em `docs/evidencias/E9/reconciliacao_numeros.txt`.
...
- **Corpus**: 6 artigos do arXiv em inglês (Lewis 2020 RAG, Karpukhin 2020 DPR, Gao 2023 survey, Es 2023 RAGAS, Asai 2023 Self-RAG, Liu 2023 Lost in the Middle) + 2 artigos em português (T12/#12, 2026-09-15: Rocha et al. 2025, RAG-SFT via SBBD; Medeiros & Oliveira 2025, comparação de embeddings/LLMs via SEMISH — decisão de quais artigos delegada ao agente pelo autor). PDFs dos artigos **não são versionados**: o README e o notebook trazem os links.
...
| `ferramentas/verificar.py e3` | Roda a verificação de uma etapa; também aceita e1_resumos, e2, e2_sobreposicao, e2_reabrir, e4, e6_ollama_desligado, e6_ollama_desligado_scripts, e6_cli_seguro, e6_fontes, e7_duplicadas e e7_estrutura |
